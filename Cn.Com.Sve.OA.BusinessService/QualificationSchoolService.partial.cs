...
namespace Cn.Com.Sve.OA.BusinessService {
    public partial class QualificationSchoolService : IQualificationSchoolService {
    	public IUnitOfWork Db { get; private set; }
    	protected SysContext SysContext{get; private set;}
    	protected IQualificationSchoolRepository Repository { get; private set; }
    	public QualificationSchoolService(SysContext ctx) {
    		this.SysContext = ctx;
    		this.Db = DbContextFactory.Create();
    		this.Repository = new QualificationSchoolRepository(this.Db);
    	}
    	public QualificationSchoolService(SysContext ctx, IUnitOfWork db) {
    		this.SysContext = ctx;
    		if(db!=null){
    			this.Db = db;
    		}else{
    	...
    	public void Save(QualificationSchool entity){
    		if(entity.Id==0){
    			this.Add(entity);
    		}else{
    			this.Db.Save();	
    	...
    	public List<QualificationSchool> FindAll() {
    	...
    	public List<QualificationSchool> FindByName(string name){
    		return this.Repository.FindByName(name).ToList();
    	}
    	public PagedModel<QualificationSchool> FindByCriteria(QualificationSchoolCriteria c) {
    		PagedModel<QualificationSchool> m = new PagedModel<QualificationSchool>();
