...
namespace Cn.Com.Sve.OA.BusinessService {
    public partial class BigInfoSourceService : IBigInfoSourceService {
    	public IUnitOfWork Db { get; private set; }
    	protected SysContext SysContext{get; private set;}
    	protected IBigInfoSourceRepository Repository { get; private set; }
    	public BigInfoSourceService(SysContext ctx) {
    		this.SysContext = ctx;
    		this.Db = DbContextFactory.Create();
    		this.Repository = new BigInfoSourceRepository(this.Db);
    	}
    	public BigInfoSourceService(SysContext ctx, IUnitOfWork db) {
    		this.SysContext = ctx;
    		if(db!=null){
    			this.Db = db;
    		}else{
    	...
    	public void Save(BigInfoSource entity){
    		if(entity.Id==0){
    			this.Add(entity);
    		}else{
    			this.Db.Save();	
    	...
    	public List<BigInfoSource> FindAll() {
    	...
    	public List<BigInfoSource> FindByName(string name){
    		return this.Repository.FindByName(name).ToList();
    	}
    	public PagedModel<BigInfoSource> FindByCriteria(BigInfoSourceCriteria c) {
    		PagedModel<BigInfoSource> m = new PagedModel<BigInfoSource>();
